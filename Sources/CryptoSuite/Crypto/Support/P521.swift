...
extension CryptoKit.P521.Signing.PublicKey: PublicKeyItem { }

extension CryptoKit.P521.Signing.ECDSASignature: SecureKeySignature { }

extension CryptoKit.P521.KeyAgreement.PrivateKey: SecureKeyConvertible {
...
extension CryptoKit.P521.Signing.PrivateKey: SecureKeyItem {
    public init(secKey: SecKey, authenticationContext: LAContext? = nil) throws(KeyConversionError) {
        var error: Unmanaged<CFError>?
        guard let data = SecKeyCopyExternalRepresentation(secKey, &error) as Data? else {
            throw .unableToExtractKeyAttributes
